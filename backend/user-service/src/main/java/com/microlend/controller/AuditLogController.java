...
public class AuditLogController {
    ...
    /** POST /api/admin/audit-logs/internal (Internal Feign endpoint) */
    @PostMapping("/internal")
    public ResponseEntity<ApiResponse<Void>> logActivity(
            @RequestParam Long userID,
            @RequestParam String action,
            @RequestParam String module) {
        auditLogService.log(userID, action, module);
        return ResponseEntity.ok(ApiResponse.success("Activity logged", null));
    }
}
