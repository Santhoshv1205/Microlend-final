...
public class UserController {
    ...
    /** GET /api/admin/users/email/{email} (Internal Feign endpoint) */
    @GetMapping("/email/{email}")
    public ResponseEntity<ApiResponse<User>> getUserByEmail(@PathVariable String email) {
        return ResponseEntity.ok(ApiResponse.success(userService.getByEmail(email)));
    }

    /** POST /api/admin/users/internal/register (Internal Feign endpoint) */
    @PostMapping("/internal/register")
    public ResponseEntity<ApiResponse<User>> registerUserInternal(@RequestBody com.microlend.dto.request.RegisterUserRequest request) {
